...
int main() {
    ...
    for (int round = 0; round < 25; round++) { // Example: 5 rounds of play
		...
		if(Player->pieces[piece_index].in_base == 1){
	    		//Take out from the base
	    		take_out_base(&players[current_player_index], 0, dice_roll);
		}
		else{
			Player->pieces[piece_index].six_counter +=1;
	    		...
	    		move_piece_forward(&players[current_player_index], 0, dice_roll);
		}
	    }
	    else{
            	// Move the player's piece 0 by the dice roll value
	    ...
	    */
	    ...
	    play(&players[current_player_index], 0, dice_roll);
